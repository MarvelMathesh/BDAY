...
const vertexShader = `
    ...
    void main() {
      ...
      // Add turbulence from mic
      float blowEffect = turbulence * 3.0; 
      ...
      // Jitter x/z based on turbulence
      pos.x += noise(vec2(time * 2.0, (position.y - time) * 4.0)) * hValue * (0.15 + blowEffect);
      pos.z += noise(vec2((position.y - time) * 4.0, time * 2.0)) * hValue * (0.15 + blowEffect);
      
      // Squash y when blowing hard
      pos.y *= 1.0 - (turbulence * 0.5 * hValue); 
...
const fragmentShader = `
    ...
    void main() {
      ...
      gl_FragColor *= strength * 3.0;
    }
  `;

export function Candle({ children, isLit = true, micVolume = 0, ...groupProps }: CandleProps) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    // Update turbulence from mic volume (smoothly)
    flameUniforms.turbulence.value = MathUtils.lerp(flameUniforms.turbulence.value, micVolume, 0.1);
    ...
    // Modulate light intensity with turbulence too
    const turbulenceFactor = 1.0 - (flameUniforms.turbulence.value * 0.5);

    light.intensity = Math.max(0, (lightStrengthRef.current + flicker * strength * 0.5) * turbulenceFactor);
